...
def RETRACT_callback(channel):
    print("PULSE")
    status()
 
def SINGLE_callback(channel):
    print("SINGLE")
    status()
 

GPIO.add_event_detect(EXTEND, GPIO.FALLING,
                      callback = EXTEND_callback,
                      bouncetime=bounce)

GPIO.add_event_detect(PULSE, GPIO.FALLING,
                      callback = PULSE_callback,
                      bouncetime=bounce)

GPIO.add_event_detect(RETRACT, GPIO.FALLING,
                      callback = RETRACT_callback,
                      bouncetime=bounce)

GPIO.add_event_detect(SINGLE, GPIO.FALLING,
...
def status():
    ...
    print("SINGLE = ", GPIO.input(SINGLE))
