...
GPIO.setup(switch_A_L, GPIO.IN, pull_up_down=GPIO.PUD_UP)
GPIO.setup(switch_B_L, GPIO.IN, pull_up_down=GPIO.PUD_UP)
...
def switch_A_L_callback(channel):
    print("PULSE")
    # status()
 
def switch_B_L_callback(channel):
    print("SINGLE")
    # status()
 

GPIO.add_event_detect(switch_A, GPIO.FALLING,
                      callback = switch_A_callback,
                      bouncetime=bounce)

GPIO.add_event_detect(switch_B, GPIO.FALLING,
                      callback = switch_B_callback,
                      bouncetime=bounce)

GPIO.add_event_detect(switch_A_L, GPIO.FALLING,
                      callback = switch_A_L_callback,
                      bouncetime=bounce)

GPIO.add_event_detect(switch_B_L, GPIO.FALLING,
                      callback = switch_B_L_callback,
                      bouncetime=bounce)
